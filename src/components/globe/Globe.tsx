...
/** Survive Globe remounts so orbit↔site never redownloads NASA textures. */
let earthTextureCache: { day: THREE.Texture; night: THREE.Texture } | null = null;
let earthTexturePromise: Promise<{ day: THREE.Texture; night: THREE.Texture }> | null = null;
  ...
  useEffect(() => {
    ...
    const load = (layer: string) =>
      new Promise<THREE.Texture>((resolve, reject) =>
        loader.load(`/api/earth-texture?layer=${layer}`, resolve, undefined, reject),
      );

    earthTexturePromise = Promise.all([load("day"), load("night")]).then(([day, night]) => {
